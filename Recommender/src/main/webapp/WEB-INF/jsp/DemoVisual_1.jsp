...
<div id="header">
<h2 align="center">Demo Visualization</h2>
...
<form id="myForm" action="${pageContext.request.contextPath}/Demo" method="POST" onsubmit="return precheck();">
<table class="center">
	<tr>
	<td>
	...
	<input type="hidden" id="Source" name="Source" value="Demo Visual" >
	<input type="hidden" id="UserID_Details" name="UserID_store" >
	<input type="hidden" id="LogDetails_SysA" name="UserLog" >
</form>
<p id="userquery" style="font-weight: bold;">${query}</p>
...
var svg = d3.select("#charthere").append("svg")
  ...
  svg.append("circle").attr("cx", 40).attr("cy", height-90).attr("r", 25).style("fill", "lightblue");
  svg.append("text").attr("x", 115).attr("y", height-120).text("Legend").style("font-weight","bold");
  svg.append("text").attr("x", 70).attr("y", height-95).text("Radius: No. of answers for that question");
  svg.append("text").attr("x", 70).attr("y", height-75).text("Opacity: TF-IDF similarity of document");
  svg.append("rect").attr("x", 15).attr("y", height-50).attr("width", 100).attr("height", 15).style("fill", "#c7c7c7");
  svg.append("text").attr("x", 15).attr("y", height-15).text("Width: Word count of concept in document");
...
<pre id="interaction_details"></pre>
</div>
